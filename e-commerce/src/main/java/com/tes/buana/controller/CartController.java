...
public class CartController extends BaseController{
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ROLE_CUSTOMER')")
    public Response<String> deleteItemCart(@PathVariable("id") String cartItemsProductId) {
        try {
            cartService.deleteItemFromCart(cartItemsProductId,userService.getUser(getUsername()));
            return ResponseHelper.ok("delete cart item product with id "+cartItemsProductId+" is success");
        } catch (Throwable e) {
